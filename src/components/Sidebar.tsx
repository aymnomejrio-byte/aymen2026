...
const NavLink: React.FC<NavLinkProps> = ({ to, icon: Icon, label, isActive }) => (
  <Button
    ...
    className={cn(
      ...
      isActive && "bg-sidebar-primary text-sidebar-primary-foreground font-semibold", // Use sidebar-primary for active state
    )}
  >
...
const SidebarContent = () => {
  ...
  const navItems = [
    ...
    { to: "/dashboard/leave-calendar", icon: CalendarDays, label: "Calendrier des Congés" },
          ...
          <SheetContent side="left" className="p-0 w-64 bg-sidebar">
            <div className="flex h-full flex-col py-4">
              <div className="px-4 mb-6">
                <h1 className="text-2xl font-bold text-sidebar-foreground">Dyad HR</h1>
              </div>
              <SidebarContent />
            </div>
          </SheetContent>
